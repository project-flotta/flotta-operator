...
function generate_go_client() {
    ...
    "${CONTAINER_RUNTIME}" run "${CONTAINER_RUNTIME_OPTS}" -u $(id -u):$(id -u) -v ${__root}:${__root}:rw,Z -v /etc/passwd:/etc/passwd -w ${__root} \
        quay.io/goswagger/swagger:v0.25.0 generate client -f ${__root}/swagger-backend.yaml --template=stratoscale    
    "${CONTAINER_RUNTIME}" run "${CONTAINER_RUNTIME_OPTS}" -u $(id -u):$(id -u) -v ${__root}:${__root}:rw,Z -v /etc/passwd:/etc/passwd -w ${__root} \
        quay.io/goswagger/swagger:v0.25.0 generate client -f ${__root}/swagger.yaml --template=stratoscale
}

function generate_go_server() {
    rm -rf restapi
    "${CONTAINER_RUNTIME}" run "${CONTAINER_RUNTIME_OPTS}" -u $(id -u):$(id -u) -v ${__root}:${__root}:rw,Z -v /etc/passwd:/etc/passwd -w ${__root} \
        quay.io/goswagger/swagger:v0.25.0 generate server  -f ${__root}/swagger-backend.yaml --template=stratoscale    
    "${CONTAINER_RUNTIME}" run "${CONTAINER_RUNTIME_OPTS}" -u $(id -u):$(id -u) -v ${__root}:${__root}:rw,Z -v /etc/passwd:/etc/passwd -w ${__root} \
