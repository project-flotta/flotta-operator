...
func (o *UpdateHeartBeatURL) Build() (*url.URL, error) {
	...
	var _path = "/namespaces/{namespace}/devices/{device-id}/heartbeat"

	deviceID := o.DeviceID
	if deviceID != "" {
		_path = strings.Replace(_path, "{device-id}", deviceID, -1)
	} else {
		return nil, errors.New("deviceId is required on UpdateHeartBeatURL")
	}
	...
	if namespace != "" {
		...
		return nil, errors.New("namespace is required on UpdateHeartBeatURL")
	}
...
func (o *UpdateHeartBeatURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UpdateHeartBeatURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UpdateHeartBeatURL")
	}
